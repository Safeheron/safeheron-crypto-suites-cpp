...
bool PDLVerifier_V2::Init(const PDLStatement &statement) {
    c_ = statement.c_;
    Q_ = statement.Q_;
...
bool PDLVerifier_V2::Step1(PDLVMessage1 &v_message1) {
    ...
    // c1 = Enc(pail_pub, a * (x + q) + b)
    safeheron::bignum::BN c1 = pail_pub_.HomomorphicMulPlain(pail_pub_.HomomorphicAdd(c_, pail_pub_.EncryptWithR(q, BN(1))), a_);
...
bool PDLVerifier_V2::Step2(const PDLPMessage1 &p_message1, PDLVMessage2 &v_message2){
...
bool PDLVerifier_V2::Accept(const PDLPMessage2 &p_message2) const {
    bool accepted = true;
    HashCommit256 sha256_com;
