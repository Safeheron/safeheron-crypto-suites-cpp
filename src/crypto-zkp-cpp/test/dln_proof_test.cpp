#include <cstring>
#include <google/protobuf/stubs/common.h>
#include <crypto-zkp/zkp.h>
#include "gtest/gtest.h"
#include "crypto-hash/sha256.h"
#include "crypto-bn/rand.h"
#include "crypto-zkp/zkp.h"
#include "crypto-paillier/pail.h"
#include "crypto-encode/base64.h"
#include "exception/located_exception.h"
#include "CTimer.h"

using std::string;
using std::vector;
using safeheron::bignum::BN;
using safeheron::curve::CurvePoint;
using safeheron::curve::Curve;
using safeheron::curve::CurveType;
using safeheron::curve::GetCurveParam;
using safeheron::hash::CSHA256;
using google::protobuf::util::Status;
using safeheron::zkp::dlog::DLogProof;
using safeheron::zkp::dlog::DLogProof;
using safeheron::zkp::heg::HegProof;
using safeheron::zkp::pail::PailProof;
using safeheron::pail::PailPubKey;
using safeheron::pail::PailPrivKey;
using safeheron::pail::CreatePailPubKey;
using namespace safeheron::zkp;
using namespace safeheron::encode;
using namespace safeheron::rand;

TEST(ZKP, DLNProof)
{
    //std::string P_hex = "";
    //std::string Q_hex = "";
    //std::string N_tilde_hex = "";

    //BN P = BN::FromHexStr(P_hex);
    //BN Q = BN::FromHexStr(Q_hex);
    //BN N_tilde = BN::FromHexStr(N_tilde_hex);

    int PRIME_BITS = 1024 ;
    BN P = RandomSafePrime(PRIME_BITS);
    BN Q = RandomSafePrime(PRIME_BITS);
    BN N_tilde = P * Q;

    BN p = (P-1)/2;
    BN q = (Q-1)/2;
    BN pq = p * q;
    BN f = RandomBNLtGcd(N_tilde);
    BN alpha = RandomBNLtGcd(N_tilde);
    BN beta = alpha.InvM(pq);

    BN h1 = ( f * f ) % N_tilde;
    BN h2 = h1.PowM(alpha, N_tilde);

    CTimer t1("dln.prove");
    dln_proof::DLNProof dln_proof_1;
    dln_proof_1.Prove(N_tilde, h1, h2, p, q , alpha);
    t1.End();

    CTimer t2("dln.verify");
    ASSERT_TRUE(dln_proof_1.Verify(N_tilde, h1, h2));

    string jsonStr;
    EXPECT_TRUE(dln_proof_1.ToJsonString(jsonStr));
    std::cout << "length(dln_proof) = " << jsonStr.length() << std::endl;
    t2.End();

    CTimer t3("dln2.prove");
    dln_proof::DLNProof dln_proof_2;
    dln_proof_2.Prove(N_tilde, h2, h1, p, q , beta);
    t3.End();

    CTimer t4("dln2.verify");
    ASSERT_TRUE(dln_proof_2.Verify(N_tilde, h2, h1));

    EXPECT_TRUE(dln_proof_2.ToJsonString(jsonStr));
    std::cout << "length(dln_proof) = " << jsonStr.length() << std::endl;
}

int main(int argc, char **argv) {
    ::testing::InitGoogleTest(&argc, argv);
    int ret = RUN_ALL_TESTS();
    google::protobuf::ShutdownProtobufLibrary();
    return ret;
}
