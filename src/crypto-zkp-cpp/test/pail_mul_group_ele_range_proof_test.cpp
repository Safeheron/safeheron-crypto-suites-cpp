#include <cstring>
#include <google/protobuf/stubs/common.h>
#include <crypto-zkp/zkp.h>
#include "gtest/gtest.h"
#include "crypto-hash/sha256.h"
#include "crypto-bn/rand.h"
#include "crypto-zkp/zkp.h"
#include "crypto-paillier/pail.h"
#include "crypto-encode/base64.h"
#include "exception/located_exception.h"
#include "CTimer.h"

using std::string;
using std::vector;
using safeheron::bignum::BN;
using safeheron::curve::CurvePoint;
using safeheron::curve::Curve;
using safeheron::curve::CurveType;
using safeheron::curve::GetCurveParam;
using safeheron::hash::CSHA256;
using google::protobuf::util::Status;
using safeheron::pail::PailPubKey;
using safeheron::pail::PailPrivKey;
using safeheron::pail::CreatePailPubKey;
using namespace safeheron::zkp;
using namespace safeheron::encode;
using namespace safeheron::rand;

TEST(ZKP, Pail_ENC_Range_Proof_3)
{
    std::string n_hex = "";
    std::string g_hex = "";
    std::string n_tilde_hex = "";
    PailPubKey pail_pub = CreatePailPubKey(n_hex, g_hex);

    BN N_tilde = BN::FromHexStr(n_tilde_hex);

    const Curve * curv = GetCurveParam(CurveType::SECP256K1);
    const CurvePoint &g = curv->g;
    BN x0 = RandomBNLt(curv->n);
    BN x = RandomBNLt(curv->n);
    BN r = RandomBNLtGcd(pail_pub.n());
    BN rho = RandomBNLtGcd(pail_pub.n());
    BN c = pail_pub.EncryptWithR(x0, r);
    BN d = ( c.PowM(x, pail_pub.n_sqr()) * rho.PowM(pail_pub.n(), pail_pub.n_sqr()) ) % pail_pub.n_sqr();
    CurvePoint X = g * x;

    BN h1 = RandomBNLtGcd(N_tilde);
    h1 = ( h1 * h1 ) % N_tilde;

    BN h2 = RandomBNLtGcd(N_tilde);
    h2 = ( h2 * h2 ) % N_tilde;

    uint32_t l = 256;
    uint32_t varepsilon = 512;

    safeheron::zkp::pail::PailMulGroupEleRangeSetUp setup(N_tilde, h1, h2);
    safeheron::zkp::pail::PailMulGroupEleRangeStatement statement(pail_pub.n(), pail_pub.n_sqr(), c, d, X, g, curv->n, 256, 512);
    safeheron::zkp::pail::PailMulGroupEleRangeWitness witness(x, rho);


    CTimer timer("time_cost_of_proof");
    safeheron::zkp::pail::PailMulGroupEleRangeProof range_proof;
    range_proof.Prove(setup, statement, witness);
    ASSERT_TRUE(range_proof.Verify(setup, statement));
    timer.End();

    std::string base64;
    range_proof.ToBase64(base64);
    safeheron::zkp::pail::PailMulGroupEleRangeProof range_proof2;
    ASSERT_TRUE(range_proof2.FromBase64(base64));
    ASSERT_TRUE(range_proof2.Verify(setup, statement));
}

int main(int argc, char **argv) {
    ::testing::InitGoogleTest(&argc, argv);
    int ret = RUN_ALL_TESTS();
    google::protobuf::ShutdownProtobufLibrary();
    return ret;
}
