...
TEST(ZKP, Pail_ENC_Range_Proof_3)
{
    ...
    CurvePoint B = curv->g * b;
    CurvePoint X = A * b + curv->g * x;
    BN c = pail_pub.EncryptWithR(x, rho);

    safeheron::zkp::pail::PailEncElGamalComRangeSetUp setup(N_tilde, h1, h2);
    safeheron::zkp::pail::PailEncElGamalComRangeStatement statement(pail_pub.n(), pail_pub.n_sqr(), c, A, B, X, curv->n, 256, 512);
    safeheron::zkp::pail::PailEncElGamalComRangeWitness witness(x, rho, a, b);


    CTimer timer("time_cost_of_proof");
    safeheron::zkp::pail::PailEncElGamalComRangeProof range_proof;
    ...
    safeheron::zkp::pail::PailEncElGamalComRangeProof range_proof2;
