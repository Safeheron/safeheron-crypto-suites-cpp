...
TEST(hash, sha512) {
    for (size_t i = 0; i < digest_message_arr.size(); ++i) {
        ...
        uint8_t digest[safeheron::hash::CSHA512::OUTPUT_SIZE];
        sha512.Write((uint8_t *)digest_message_arr[i][1].c_str(), digest_message_arr[i][1].length());
        sha512.Finalize(digest);
        EXPECT_TRUE(bytes2hex(digest, safeheron::hash::CSHA512::OUTPUT_SIZE) == digest_message_arr[i][0]);
    }
}
