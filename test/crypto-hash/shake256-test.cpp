...
#include "shake256_vectors_pretty.h"

using ssgx_test::utils::BytesToHex;
using ssgx_test::utils::HexToBytes;

TEST(hash, shake256) {
    for (const auto& vec : shake256_vectors) {
        ...
        safeheron::hash::CShake256 shake256(out_len);
        shake256.Write(input_bytes.data(), input_byte_len);
        std::vector<uint8_t> digest = shake256.Finalize();
